...
**Context:** "Offroad Parks" was always a working codename. Before we can acquire a domain and form an LLC we need a **true identity**. Sprint 8 research (see [BRANDING.md](BRANDING.md) + [BRANDING-merch-plan.md](BRANDING-merch-plan.md)) evaluated two paths — distinctive/ownable (e.g. "Rutwild") vs descriptive/SEO — and the founder chose **`OHVAlmanac`** (descriptive: an *almanac of OHV areas*; best-vetted, fits the **vintage/heritage badge** aesthetic). `ohvalmanac.com` + all defensive TLDs + IG/TikTok handles confirmed open (2026-07-16). Tradeoff accepted: descriptive = a weak, hard-to-enforce mark — differentiation via brand + content, not legal exclusivity. The merch strategy centers on **Earn-a-Badge** — riders earn a collectible physical + digital badge for logging a park visit, turning merch into the acquisition funnel and a retention loop (bridges to E11 gamification: OP-57 visit tracking / OP-58 achievements).
...
| Key | Title | Status | Type | Notes |
|-----|-------|--------|------|-------|
| OP-104 | Brand Name Selection | done | decision | ✅ **`OHVAlmanac`** selected 2026-07-16 (Path B, descriptive). Beat Rutwild (distinctive path) and OHVCompass; OHVAtlas/OffroadBeacon dropped on same-space conflicts. See BRANDING.md §5–6. |
| OP-105 | Formal Trademark Clearance | blocked | infra | 🔴 Requires a trademark attorney: full clearance + intent-to-use in Nice classes **9/42/41/25** (consider 16/35/39). Descriptive mark → likely Supplemental Register initially. Clearance brief in BRANDING.md §6a. Our checks are preliminary only. Blocks domain/LLC/merch spend. |
| OP-106 | Domain Acquisition | blocked | infra | `ohvalmanac.com` + all defensive TLDs + IG/TikTok handles **confirmed open** (2026-07-16) — register once cleared (OP-105). Checklist in BRANDING.md §6b. Set `NEXT_PUBLIC_SITE_URL` (unblocks OP-103 in E22). |
