...
| OP-78 | Source Reliability Tiers & Domain Lists | backlog | feature | New DomainReliability model (domainPattern, defaultReliability, isBlocked, notes). Seeded with known tiers: `.gov`=85, `riderplanet-usa.com`=90, `facebook.com`=60, known-bad registries=blocked. Admin UI section on sources page to add/edit/remove domain entries. Source discovery pipeline checks domain table when assigning reliability to new sources — match → inherit, no match → default 50. |
| OP-79 | Extraction Accuracy Feedback Loop | backlog | feature | On each approve/reject of a FieldExtraction, increment approve_count or reject_count on the linked DataSource. Compute accuracy rate (approves / total). Aggregate per domain and update DomainReliability entries (OP-78). Sources below 20% accuracy flagged with red badge in admin UI (not auto-skipped — admin decides). Dashboard widget showing domain accuracy trends over time. |
...
| OP-81 | Wrong-Park Detection Guard | backlog | feature | Add a dedicated LLM validation call per source before extraction: "Is this content primarily about [park name] in [state]? Answer yes/no with a brief reason." If no → skip extraction and mark source WRONG_PARK automatically. Adds one cheap LLM call per source but prevents wasted extraction tokens + bad data. Manual wrong-park button (OP-77a) remains as admin override. |
| OP-82 | Extraction Validation Rules | backlog | feature | Post-extraction validation before writing FieldExtraction records. Rules: (1) lat/lng must be within the park's state bounding box, (2) prices $0–$500 range, (3) phone numbers 10+ digits, (4) URL fields are valid URL format (no HTTP resolution — too slow), (5) enum arrays already enforced by Zod schema. Fields failing validation are dropped from that extraction with a warning logged on the session, not surfaced in review queue. |
...
| Key | Title | Status | Type | Notes |
|-----|-------|--------|------|-------|
| OP-85 | Park Discovery: State-Level Search | backlog | feature | New ParkCandidate model (name, state, city, estimatedLat, estimatedLng, sourceUrl, status: PENDING/ACCEPTED/REJECTED). SerpApi queries per state ("OHV parks in [state]", "off-road riding areas [state]"). LLM `generateObject()` extracts structured candidates (name, city, estimated coordinates) from search snippets. Fuzzy dedup against existing parks (Levenshtein on name + same state). Returns candidate list for admin review (OP-86). |
| OP-86 | Park Discovery: Admin Review & Seeding | backlog | feature | New admin page /admin/ai-research/discovery. Table of ParkCandidate records grouped by state, showing name, city, estimated coordinates, source URL. Accept → creates seed Park record (APPROVED status, NEEDS_RESEARCH researchStatus) with name, state, city, coordinates. Reject → marks candidate REJECTED (excluded from future discovery). Bulk accept/reject. After accept, auto-triggers researchPark() for the new park (OP-87). |
